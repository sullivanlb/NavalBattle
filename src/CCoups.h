/*
 *	Cette classe mémorise tous les tirs é la fois du joueur et de l'adversaire :
...
class CCoups {
	...
	private:
		 ...
		 * Note importante : pour connaître les dégats sur les bateaux du joueur, consulter le tableau m_degats de chaque
		 * bateau du joueur.
		 */
	...
	public:
		...
		bool tirDansMap ( const string& s, pair<int,int>& p );		// renvoie un booléen à vrai si le tir passé en paramétre et associé à une clé est dans la structure m_donnees
        
		void ajouterTirMap ( const string& s, pair<int,int>& p );	// ajoute un nouveau tir "p" dans la collection associée à la clé "s" passée en paramétre
